...
import policeIconImg from '../../assets/img/tablets/fib/fib-logo.png'
...
class FibTablet extends React.Component {
    ...
    sendSearchPlayer(e) {
        ...
        if (pageE.step === 1 || pageD.step === 1 || pageB.step === 0) {
            ...
            alt.emit('tablet.fib.searchPlayer', 'name', param)
        } else if (pageE.step === 2 || pageD.step === 2 || pageB.step === 2) {
            alt.emit('tablet.fib.searchPlayer', 'playerId', playerId)
        }
    }
    ...
    giveFine(e) {
        ...
        e.preventDefault()
        alt.emit('tablet.fib.giveFine', searchPlayer, reason, summ)
    ...
    giveWanted(e) {
        ...
        e.preventDefault()
        alt.emit('tablet.fib.giveWanted', searchPlayer, stars)
    ...
    componentDidMount() {
        alt.on('fibTablet', (event, options) => {
            const { calls, playersOnline, FIBPage } = this.state
            if (event === 'addCall') {
                ...
                window.fibTablet.enable(options)
    ...
    removeCall = (id, x, y) => {
        alt.emit('tablet.fib.acceptCall', id, x, y)
    }

    callHelp = event => {
        if (event === 'callTeamHelp') {
            alt.emit('tablet.fib.callTeamHelp')
        } else if (event === 'callHospitalHelp') {
            alt.emit('tablet.fib.callHospitalHelp')
        } else if (event === 'callFibHelp') {
            alt.emit('tablet.fib.callFibHelp')
        }
    }
                    ...
                    <div className="content">
                                ...
                                <div className="row">
                                    ...
                                    <div className="column">
                                        ...
                                        <div className="description">
                                            Используйте базу данных Federal Investigation Bureau,
                        ...
                        {(FIBPage === 'B')
                            && <React.Fragment>
                                {pageB.step === 0 && searchPlayer === null
                                    && <div className="addWanted">
                                        ...
                                        <div className="help-list">
                                            <button
                                                onClick={() => this.handleOnClickEB(1)}
                                                type="button"
                                            >
                                                По имени и фамилии
                                            </button>
                                            <button
                                                onClick={() => this.handleOnClickEB(2)}
                                                type="button"
                                            >
                                                По номеру паспорта
                                            </button>
                                ...
                                {(pageB.step === 1 && searchPlayer === null)
                                    && <div className="addWanted">
                                        <div className="addWanted-title">
                                            Найти гражданина
                                        </div>
                                ...
                                {(pageB.step === 2 && searchPlayer === null)
                                    && <div className="addWanted">
                                        <div className="addWanted-title">
                                            Выписать штраф
                                        </div>
                                        <div className="addWanted-text">
                                            Используйте технологию поиска по номеру паспорта.
                                            ориентировки. Это новая функция для FIB.
                                            С помощью неё вы сможете узнать
                                            частично информацию о любом жильце в штате
                                        </div>
                                        <form>
