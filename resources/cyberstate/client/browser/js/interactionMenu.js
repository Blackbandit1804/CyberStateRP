$(document).ready(() => {
    ...
    window.interactionMenuAPI = {
        ...
        addItemFactionInventory: (iconName, text, itemCheck, clear) => {
          ...
          let haveItem = window.inventoryAPI.getArrayByItemId(parseInt(itemCheck))
...
function onClickPlayerItem(itemName) {
    ...
    alt.emit(`interactionMenu.onClickPlayerItem`, itemName);
}
...
function onClickVehicleItem(itemName) {
    ...
    if (itemName == "Организация") {
        ...
        alt.emit(`events.emitServer`, `removeFromVehicle`, itemName);
        interactionMenuAPI.hide();
    }
    alt.emit(`interactionMenu.onClickVehicleItem`, itemName);
}


alt.on(`interactionMenuAPI.addItemFactionInventory`, (iconName, text, itemCheck, clear) => {
    interactionMenuAPI.addItemFactionInventory(iconName, text, itemCheck, clear)
});

alt.on(`interactionMenuAPI.showPlayerMenu`, (data) => {
    //console.log(data);
    interactionMenuAPI.showPlayerMenu(data);
});

alt.on(`interactionMenuAPI.showVehicleMenu`, (data) => {
    interactionMenuAPI.showVehicleMenu(data);
});

alt.on(`interactionMenuAPI.hide`, () => {
    interactionMenuAPI.hide();
});

alt.on(`interactionMenuAPI.move`, (x, y) => {
    interactionMenuAPI.move(x, y);
});
