$(document).ready(() => {
    ...
    window.consoleAPI = {
        ...
        send: (message) => {
            alt.emit('console.send', message.escape());
            //alt.invoke("command", message.escape());
        },
        sendMessage: (message) => {
            var reportId = $("#console .report-dialog").data("sqlid");
            if (!reportId) return nError(`ID тикета не найден!`);
            if (!isFlood()) alt.emit("events.emitServer", "report.addMessage", JSON.stringify([reportId, message.escape()]));
        },
        sendChat: (message) => {
            if (!isFlood()) alt.emit("events.emitServer", "admin.chat.push", JSON.stringify([message.escape()]));
        },
        show: (enable) => {
            ...
            if (enable) {
                ...
                alt.emit("setConsoleActive", enable)
        ...
        showReportMenu: (sqlId, left, top) => {
            ...
            menu.forEach((menuItem) => {
                var menuItemEl = $(`<div class="menuitem">${menuItem}</div>`);
                menuItemEl.click((e) => {
...
var reportMenuHandlers = {
    "Закреп": (reportId) => {
        alt.emit(`events.emitServer`, "report.attach", reportId);
    },
    "Закрыть": (reportId) => {
        alt.emit(`events.emitServer`, "report.close", reportId);
    },
    "ТП к игроку": (reportId) => {
        alt.emit(`events.emitServer`, "report.goto", reportId);
    },
    "ТП к себе": (reportId) => {
        alt.emit(`events.emitServer`, "report.gethere", reportId);
    },
};

alt.on(`consoleAPI.pushReport`, (data) => {
    consoleAPI.pushReport(data);
});

alt.on(`consoleAPI.removeReport`, (sqlId) => {
    consoleAPI.removeReport(sqlId);
});

alt.on(`consoleAPI.addReportMessage`, (sqlId, messages) => {
    consoleAPI.addReportMessage(sqlId, messages);
});

alt.on(`consoleAPI.chat`, (player, text) => {
    consoleAPI.chat(player, text);
});

alt.on(`consoleAPI.setReportAdminId`, (sqlId, adminId) => {
    consoleAPI.setReportAdminId(sqlId, adminId);
});

alt.on(`consoleAPI.enable`, (enable) => {
    consoleAPI.enable(enable);
});

alt.on(`consoleAPI.log`, (text) => {
    consoleAPI.log(text.escape());
});

alt.on(`consoleAPI.info`, (text) => {
    consoleAPI.info(text.escape());
});

alt.on(`consoleAPI.warning`, (text) => {
    consoleAPI.warning(text.escape());
});

alt.on(`consoleAPI.error`, (text) => {
    consoleAPI.error(text.escape());
});

alt.on(`consoleAPI.debug`, (text) => {
    consoleAPI.debug(text.escape());
});