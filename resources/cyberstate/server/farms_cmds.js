module.exports = {
    "farm_field_fill": {
        ...
        handler: (player, args) => {
            var field = alt.farmFields.getBySqlId(args[0]);
            if (!field) return terminal.error(`Поле с ID: ${args[0]} не найдено!`, player);
            args[1] = Math.clamp(args[1], 0, 3);

            field.fill(args[1]);
            terminal.info(`${player.getSyncedMeta("name")} засеял поле с ID: ${args[0]}`)
        }
    },
