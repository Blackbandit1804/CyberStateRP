...
let rooberColShape = new alt.ColshapeSphere(158.976, -3082.372, 6.014, 1.5); // Колшейп для взаимодействия с NPC
...
function DestoryRooberPlace(player) {
    try
    {
        if (getRooberPlace(player) !== undefined) {
            alt.emitClient(player, "destroyRooberPlace");
            if (getRooberPlace(player).vehicle !== undefined) {
                if (player.vehicle) if (player.vehicle.roober !== undefined) if (player.vehicle.roober === player) alt.emitClient(player, `Vehicle::leave`, player.vehicle);
...
async function CreateRooberPlace(player) {
    try
    {
        if (getRooberPlace(player) !== undefined) {
        ...
        let veh = await alt.gameEntityCreate(vehname, new alt.Vector3(pos.x, pos.y, pos.z), new alt.Vector3(0, 0, pos.rot * Math.PI / 180));
        
        veh.numberPlateText = "Cyber State";
        veh.customPrimaryColor = [col, col, col];
        veh.customSecondaryColor = [col, col, col];
        veh.dimension = 1;
        veh.roober = player;
        ...
        setSaveTimeout(function() {
          try {
            ...
            alt.emitClient(player, "createRooberPlace", pos.x, pos.y, pos.z, rooberinfo.max_range[rangenum].time,  vehname, veh, savemoney);
          } catch (err) {
              alt.log(err);
              return;
          }
...
alt.onClient("create.roober.place", (player) => {
    try
    {
...
alt.onClient("pass.roober.place", (player, money) => {
    try
    {
...
alt.onClient("destroy.roober.place", (player) => {
    try
    {
        DestoryRooberPlace(player);
    } catch (err){
        ...
        if (vehicle.roober !== undefined) {
            if (vehicle.roober === player) {
               if (vehicle.rooberlock === undefined) {
                   alt.emitClient(player, "startRoobingVehicle", getRandomNumber(50, 100));
    ...
    try
    {
        if (!player.vehicle) {
            if (shape == rooberColShape) {
                ...
                player.setSyncedMeta("keydownevariable2", true);
            }
        } else {
...
alt.on("entityLeaveColshape", (shape, player) => {
    ...
    {
      if (shape == rooberColShape) {
        player.setSyncedMeta("keydownevariable2", undefined);
        alt.emitClient(player, "hideRooberMenu");
      }
    } catch (err) {
        alt.log(err);
        return;
...
alt.log("[CarRoober] CarRoober in Los Santos started!");
