module.exports = {
    "clothes_variation": {
        ...
        handler: (player, args) => {
            ...
            var com = alt.getClothes(args[1], args[0]);
            if (!com) return terminal.error(`Компонент одежды не найден!`, player);

            com.variation = args[2];
            DB.Query(`UPDATE store_${args[1]} SET variation=? WHERE id=?`, [args[2], args[0]]);
            terminal.info(`${player.getSyncedMeta("name")} обновил variation у одежды с ID: ${args[0]}`);
        }
    },
    "clothes_price": {
        ...
        handler: (player, args) => {
            ...
            com.price = args[2];
            DB.Query(`UPDATE store_${args[1]} SET price=? WHERE id=?`, [args[2], args[0]]);
            terminal.info(`${player.getSyncedMeta("name")} обновил price у одежды с ID: ${args[0]}`);
        }
    },
    "clothes_size": {
        ...
        handler: (player, args) => {
            ...
            DB.Query(`UPDATE store_${args[1]} SET rows=?,cols=? WHERE id=?`, [com.rows, com.cols, args[0]]);
            terminal.info(`${player.getSyncedMeta("name")} обновил rows и cols у верхней одежды с ID: ${args[0]}`);
        }
    },
    "clothes_addtexture": {
        ...
        handler: (player, args) => {
            ...
            DB.Query(`UPDATE store_${args[1]} SET textures=? WHERE id=?`, [JSON.stringify(com.textures), args[0]]);
            terminal.info(`${player.getSyncedMeta("name")} обновил textures у ${args[1]} одежды с ID: ${args[0]}`);
        }
    },
